...
# Mock ADF results format: (statistic, p-value, usedlag, nobs, critical values, icbest)
...
def test_adf_test_failure_raises_value_error(mock_adf):
    ...
    with pytest.raises(ValueError, match="ADF test failed"):
        adf_test(pd.DataFrame([]))  # or any invalid input
...
# Mock results structure from grangercausalitytests
mock_result_causal = {
