...
class ConfigLoader:
    def __init__(self):
        self.config_filename = os.environ.get("CONFIG_FILENAME", f"{get_root_directory()}\\configs\\pipeline_template.yml")
    ...
    def define_get_raw_data(self)-> dict[str, Any]:
        ...
        """
        get_raw_data_config = [stage for stage in self.config.get("pipeline",{}) if stage.get("stage")=="get_raw_data"]
        if not get_raw_data_config:
            get_raw_data_config = [{}]
        else:
            if get_raw_data_config[0].get("tickers") is None and get_raw_data_config[0].get("active") is True:
    ...
    def define_process_raw_data(self)-> dict[str, Any]:
        ...
        """
        process_raw_data_config = [stage for stage in self.config.get("pipeline",{}) if stage.get("stage")=="process_raw_data"]
        if not process_raw_data_config:
            process_raw_data_config = [{}]
        else:
            if process_raw_data_config[0].get("tickers") is None and process_raw_data_config[0].get("active") is True:
                logger.error("Tickers not found in config file")
                raise ValueError("Tickers not found in config file.")
    ...
    def define_conduct_sentiment_analysis(self)-> dict[str, Any]:
        ...
        """
        conduct_sentiment_analysis_config = [stage for stage in self.config.get("pipeline",{}) if stage.get("stage")=="conduct_sentiment_analysis"]
        if not conduct_sentiment_analysis_config:
            conduct_sentiment_analysis_config = [{}]
        else:
            if conduct_sentiment_analysis_config[0].get("sources") is None and conduct_sentiment_analysis_config[0].get("active") is True:
    ...
    # TODO
    def define_run_model(self):
        pass

    # TODO
    def define_get_results(self):
        pass
