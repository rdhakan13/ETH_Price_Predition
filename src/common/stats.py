...
def grangers_causality_test(
    df: pd.DataFrame, target_col: str, max_lag: int = 5, alpha: float = 0.05
) -> dict[str, Any]:
    """
    ...
    for col in df.columns:
        ...
        try:
            test_result = grangercausalitytests(
                df[[target_col, col]].dropna(), max_lag, verbose=False
            )
        except Exception as e:
            logger.error(f"Error in Granger's Causality test for {col}: {e}")
            raise ValueError(
                f"Granger's Causality test failed for {col}. Please check the input data."
            )

        p_values = {
            lag: test_result[lag][0]["ssr_chi2test"][1] for lag in range(1, max_lag + 1)
        }
